...
contract("SupplyChain", (accounts) => {
  ...
  it("should add an item with a quantity", async () => {
    ...
    assert.equal(item.quantity.toString(), "10", "Item quantity mismatch");
    assert.equal(item.state.toString(), "1", "Item state should be ForSale");
  ...
  it("should allow a buyer to purchase an item and reduce quantity", async () => {
    ...
    const item = await instance.items(1);
    assert.equal(item.quantity.toString(), "8", "Item quantity should decrease by 2");
    assert.equal(item.state.toString(), "1", "Item state should remain ForSale if quantity > 0");
  });
  ...
  it("should mark the item as Sold when quantity becomes zero", async () => {
    ...
    const item = await instance.items(1);
    assert.equal(item.quantity.toString(), "0", "Item quantity should be zero");
    assert.equal(item.state.toString(), "2", "Item state should be Sold when quantity is zero");
  });

  it("should allow the seller to ship the item", async () => {
    await instance.shipItem(1, { from: seller });
    const item = await instance.items(1);
    assert.equal(item.state.toString(), "3", "Item state should be Shipped");
  });

  it("should allow the buyer to receive the item", async () => {
    await instance.receiveItem(1, { from: buyer });
    const item = await instance.items(1);
    assert.equal(item.state.toString(), "4", "Item state should be Received");
  });
});
